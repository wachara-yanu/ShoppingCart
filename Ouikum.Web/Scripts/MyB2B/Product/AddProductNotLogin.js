...
function GetValueTagit() {
    ...
    $('#tagit-detail').find(".tagit-choice").each(function (index) {
        //var kw = index - 1;
        //ProductDetail += $(this).text().replace("x", "~");
        var kw = index - 1;
        ProductDetail += $(this).text();
        ProductDetail += "~";
    });
...
function GoToPageStep(currentpage, pageto, save) {
    ...
    if (currentpage <= pageto) {
        //      next step
        if (currentpage == 1) {
            bool = chcekAddProductPageOne();       // check error on page one
            if (bool)
                used1 = true;
        } else if (currentpage == 2) {
            bool = chcekAddProductPageTwo();       // check error on page two
            if (bool)
                used2 = true;
        }

        if (bool == true && save == true)          //save is not error 
            SaveProduct();

        if ((currentpage == 1 && used1 == true) ||
...
function chcekAddProductPageOne() {               // Page Add Product
    ...
    if ($('#TextSearchCategory').val().length > 1) {
        console.log('TextSearchCategory  true');
        $("#Category").closest('.control-group').addClass('success');
        $("#Category").closest('.control-group').removeClass('error');
        //  $("#Category .successImg").css('display', 'block');
        $(".div-sel-category").css('display', 'none');
        ...
        console.log('TextSearchCategory  false');
        $("#Category").closest('.control-group').removeClass('success');
        $("#Category").closest('.control-group').addClass('error');
        $("#TextSearchCategory").attr('placeholder', label.vldrequired);
        bool = false;
...
function chcekAddProductPageThree()    // Page Register
{
    ...
    if ($('#signup_form').valid()) {
        ...
        //var Slider = object.setAttribute("aria-disabled", value);
        //var Slider = object.getAttribute("aria-disabled");
        //console.log('isAgree ' + isAgree);

        if (UserName && Email && CompName && Agree)
            bool = true;
        else
            bool = false;
        ...
        //if (Slider == "false") {
        //    bootbox.alert('กรุณาเลื่อนแถบไปด้านขวา เพื่อปลดล๊อค');
        //    bool = false;
        //}
        //if (Slider == false) {
        //    bootbox.alert('กรุณาเลื่อนแถบไปด้านขวา เพื่อปลดล๊อค');
...
function compareKeyword(kw) {
    var len = $('#tagit-keyword .tagit-choice').length;
    for (var i = 0; i < len; i++) {
        var val = $('.tagit-choice').eq(i).text();
        if (val == kw) {
            return false;
        }
    }
...
function addmorekeyword(Obj) {
    ...
    $('#tagit-keyword .tagit-choice').remove();
