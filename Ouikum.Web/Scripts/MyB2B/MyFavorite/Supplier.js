//autoComplete('/MyB2B/Favorite/GetCompName');
...
function Onload() {
    OpenLoading(true);
    $.ajax({
        url: GetUrl("MyB2B/Favorite/Suppliers"),
        data: data,
...
function ChooseComp(obj) {
    ...
    if (obj != undefined || obj != null) {
        if (obj.attr("checked") == true || obj.attr("checked") == "checked") {
            ...
            obj.removeClass("Sup-data");
            obj.removeAttr("checked", "checked");
        }
    } else {
        for (var i = 0; i < (".ChooseComp").length; i++) {
            ChooseComp($(".ChooseComp").eq(i - 1));
            ChooseComp($(".ChooseComp").eq(i - 1));
        }
    }
}
...
function MultiDel(Obj) {
    ...
    bootbox.confirm(label.vldconfirm_del_data, "Cancel", "Yes", function (e) {
        if (e) {
            GetCompDel();
            $.ajax({
                url: GetUrl("MyB2B/Favorite/DeleteFavComp"),
                data: { FavSupplierID: CompDel },
                traditional: true,
                success: function (data) {
        ...
        else { OpenLoading(false); }
    })
}
var CompDel = new Array();
//============ Get Product For Delete Item ===================//
function GetCompDel() {
    var len = $('.Sup-data').length;
    for (var i = 0; i < len; i++) {
        CompDel[i] = $('.Sup-data').eq(i).attr('data-id');
    }
    console.log(CompDel);
}
function CheckCompAll(Obj) {
    if (Obj.attr("checked") == true || Obj.attr("checked") == "checked") {
        $(".ChooseComp").attr("checked", "checked");
        $(".ChooseComp").addClass("Sup-data");
        $(".divProductItem").addClass("hdivProductItem");
        $(".divProductItemG").addClass("hdivProductItem");
    } else {
        $(".ChooseComp").removeAttr("checked");
        $(".ChooseComp").removeClass("Sup-data");
...
function DeleteFav(Obj) {
    ...
    bootbox.confirm(label.vldconfirm_del_data, "Cancel", "Yes", function (e) {
        if (e) {
            $.ajax({
                url: GetUrl("MyB2B/Favorite/DeleteFavComp"),
                data: { FavSupplierID: Obj.attr("data-id") },
                traditional: true,
                success: function (data) {
